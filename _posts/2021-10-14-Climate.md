...
```
    ...
    <sqlite3.Cursor at 0x7fbd25af79d0>
...
```
    ...
    /Users/yiningliang/anaconda3/lib/python3.8/site-packages/pandas/core/generic.py:2779: UserWarning:
...
def coef(data_group):
    """
    This function serves to build a linear regression model.
    It outputs the first coefficient, which will be used to make a plot.
    """
...
def temperature_coefficient_plot(country, year_begin, year_end, 
    ...
    """
    The function serves to create an interactive geographic scatterplot, 
    with a point for each station, 
    such that the color of the point reflects 
    an estimate of the yearly change in temperature 
    ...
    Only data for stations with at least min_obs years worth of data 
    in the specified month are plotted; 
    the others are filtered out. 
    - **kwargs, additional keyword arguments passed to px.scatter_mapbox(). 
    These can be used to control the colormap used, the mapbox style.
    """
...
* **Could I see two countries' changing trend in multiple axes?**

We want to create a scatter plot with multiple axes in this section. This plot aims to show the average annually temperature of the countries within a time period. 

First, we need to subset the dataframe according to the `country 1`, `country 2`, `year_begin`, `year_end`. We did not round the temperature value since the yearly average temperature vary little, and a more accurate value could help to detect the subtle change. But, of course, you could choose to round the values to make the value presented in a more formatted way.


```python
def plot2_sql_query(country1, country2, year_begin, year_end):
    """
    This function serves to filter the data with user-specified arguments:
    the filtered dataframe contains data 
    in two certain countries from the year_begin to year_end.
    The function prepares the data for scatter plotting.
    """
...
def scatter_average(country1,country2, year_begin, year_end,**kwargs):
    """
    The function serves to create an interactive scatterplot, 
    showing the yearly average temperature in countires,
    with different countries' data plotted in different axes.
    The color of the points indicate temperatures.
    
    The function accepts four arguments: 
    - country1, a string giving the name of a country 
    for which data should be returned.
    - country2, a string giving the name of a second country 
    for which data should be returned.
    - year_begin and year_end, two integers 
    giving the earliest and latest years 
    for which should be returned.
    ...
    fig = px.scatter(myscatter, 
                     x = "Year",
                     y = "Temp", 
                     color = "Temp",
                     facet_col = "Country",
                     color_continuous_midpoint = 0,
                     title = "Scatterplot showing "+str(country1)+
                     " and " +str(country2)+ " Temperature throughout "
                     + str(year_begin)+" and "+str(year_end),**kwargs)
...
```python
fig = scatter_average("India", "Iceland", 1980,2020)
...
```python
fig = scatter_average("China", "Japan", 1980,2020)
...
def z_score(x):
    """
    This function serves to compute z score.
    """
    m = np.mean(x)
    s = np.std(x)
    return (x - m)/s
...
def to_heat(country, year_begin, year_end):
    """
    This function serves to prepare the data for heatmap plotting.
    The dataframe is subset according to the user-specified country name,
    the earliest and the latest year to be returned.
    """
...
Now, we start to write the functiion for plotting the z scores on the heatmap.
...
def heatmap (country,year_begin, year_end):
    """
    The function serves to create an interactive heatmap, 
    visualizing the z score for a country throughout the user-specified
    year_begin and year_end every month.
    ...
    fig = px.imshow(heat, 
                    labels = dict(x = "Month", y = "Year", color = "z"),
